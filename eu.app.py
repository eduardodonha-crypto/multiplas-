...
Fontes: API-Football (principal) + football-data.org (extra)
Máximo de ligas e jogos possíveis no plano gratuito.
"""
...
# ============================================================
# LIGAS API-FOOTBALL (expandido)
# ============================================================

LIGAS_AF = {
    39: "Premier League", 140: "La Liga", 78: "Bundesliga", 135: "Serie A",
    61: "Ligue 1", 71: "Brasileirão Série A", 72: "Brasileirão Série B",
    ...
    128: "Liga Profesional (Argentina)", 130: "Primera Nacional (Arg)",
    262: "Liga MX", 253: "MLS", 203: "Süper Lig",
    144: "Belgian Pro League", 179: "Scottish Premiership",
    ...
    286: "Serbian SuperLiga", 292: "Romanian Liga 1", 235: "Russian Premier",
    169: "Chinese Super League", 98: "J1 League", 292: "K League 1",
    307: "Saudi Pro League", 188: "A-League", 253: "MLS",
    71: "Brasileirão", 13: "Copa Libertadores", 11: "Copa Sudamericana",
    15: "FIFA Club World Cup", 1: "World Cup", 4: "Euro",
}
...
def af_jogos(ligas_ids):
    ...
    for lid in ligas_ids:
        ...
        for item in data.get("response", []):
            if len(jogos) >= MAX_JOGOS: break
            fx = item.get("fixture", {})
            if fx.get("status", {}).get("short") in ("FT","AET","PEN","PST","CANC","ABD"):
                continue
...
def af_forma(team_id):
    ...
    for j in js:
        ...
        if gc+gf > 2.5: over += 1
    n = len(js)
    f = {"win": round(vit/n*100,1), "btts": round(btts/n*100,1), "over": round(over/n*100,1)}
...
def af_odds(fid):
    ...
    try:
        for b in data["response"][0].get("bookmakers",[{}])[0].get("bets",[]):
...
# ============================================================
# FOOTBALL-DATA.ORG (fonte extra)
# ============================================================

def fd_get(endpoint, params=None):
    if API_KEY_FD in ("SUA_CHAVE_FD", None, ""):
        return None
...
def fd_jogos():
    """Jogos de hoje das 12 ligas gratuitas do football-data.org"""
    data = fd_get("matches", {"dateFrom": hoje(), "dateTo": hoje()})
    if not data: return []
    jogos = []
    for m in data.get("matches", []):
        if m.get("status") in ("FINISHED","AWARDED"): continue
        home = m.get("homeTeam", {})
        away = m.get("awayTeam", {})
...
def analisar(j):
    ...
    if fonte == "AF":
        ...
        # football-data.org: forma simplificada (sem histórico detalhado no free)
        fh = fa = {"win": 45.0, "btts": 50.0, "over": 50.0}  # neutro
        odds = {}
...
st.caption(f"{agora().strftime('%d/%m/%Y %H:%M')} • API-Football + football-data.org")

if API_FOOTBALL_KEY == "SUA_CHAVE_API_FOOTBALL":
    st.error("Configure API_FOOTBALL_KEY nos Secrets.")
    st.stop()

with st.expander("⚙️ Ligas API-Football", expanded=False):
    nomes = list(dict.fromkeys(LIGAS_AF.values()))  # unique
    sel = st.multiselect("Selecione (quanto mais, mais requests)", nomes, default=nomes[:15])
    ids = [k for k,v in LIGAS_AF.items() if v in sel]

usar_fd = st.checkbox("Incluir football-data.org (12 ligas extras)", value=True)

if st.button("🔄 Gerar sugestões", type="primary"):
    with st.spinner("Buscando em múltiplas fontes..."):
        jogos = af_jogos(ids) if ids else []
        if usar_fd:
            jogos += fd_jogos()
        ...
        if not jogos:
            st.warning("Nenhum jogo encontrado hoje. Tente outras ligas ou volte mais tarde.")
            st.stop()

        st.info(f"{len(jogos)} jogo(s) de {len(set(j['liga'] for j in jogos))} ligas")
        cands = []
        bar = st.progress(0)
        ...
        st.session_state.ultima = {"top": top, "multiplas": multis}
        st.success("Pronto!")
...
if "ultima" in st.session_state:
    ...
    for i, s in enumerate(top, 1):
        ...
        st.markdown(f"**{i}. [{s.liga}] {s.jogo}** — {s.mercado}  \n**{s.confianca_pct}%** | {o} | {s.fonte}")

    for m in multis:
        st.subheader(m.nome)
        st.caption(m.descricao)
        st.markdown(f"Odd total **{m.odd_total}** | Acerto **{m.chance_real}%** | Média {m.confianca_media}%")
        for s in m.selecoes:
            o = s.odd_real or s.odd_estimada
            st.markdown(f"- [{s.liga}] {s.jogo}: **{s.mercado}** ({s.confianca_pct}% | odd {o})")

    rows = [{"multipla":m.nome,"liga":s.liga,"jogo":s.jogo,"mercado":s.mercado,
             "confianca":s.confianca_pct,"odd":s.odd_real or s.odd_estimada,"fonte":s.fonte}
...
st.caption("Duas fontes • Máx. 50 jogos • Cache ativo")
